...
S3_BACKUP_KEY="${S3_BACKUP_KEY:-database-backup.sql}"  # This will be overwritten each time
...
backup_database() {
    ...
    if [ "$DB_TYPE" = "mysql" ]; then
        mysqldump \
            ...
            --quick \
            --lock-tables=false \
            "$DB_NAME" > "$BACKUP_FILE" 2>> "$LOG_FILE" || error_exit "MySQL backup failed"
        ...
        PGPASSWORD="$DB_PASSWORD" pg_dump \
            ...
            --no-owner \
            --no-acl \
            --file="$BACKUP_FILE" 2>> "$LOG_FILE" || error_exit "PostgreSQL backup failed"
    fi
...
upload_to_s3() {
    log "Uploading backup to S3-compatible storage..."
    ...
    # Upload with overwrite (no versioning needed)
    aws s3 cp "$BACKUP_FILE" \
        "s3://${S3_BUCKET}/${S3_BACKUP_KEY}" \
        $endpoint_arg \
        --region "$S3_REGION" \
        2>> "$LOG_FILE" || error_exit "S3 upload failed"
    
    log "Backup uploaded successfully to s3://${S3_BUCKET}/${S3_BACKUP_KEY}"
}
...
cleanup() {
    ...
    find "$BACKUP_DIR" -name "backup_${DB_NAME}_*.sql" -type f -mtime +1 -delete 2>/dev/null || true
